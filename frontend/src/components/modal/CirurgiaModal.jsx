...
import { User, FileText, Calendar, Clock, UploadCloud, FileCheck, X, Eye, Trash2, CheckSquare, Syringe, CalendarDays, MapPin, Paperclip } from "lucide-react";
...
const CustomCheck = ({ label, field, checked, onChange }) => (
    <label style={{ display: "flex", alignItems: "center", gap: "12px", cursor: "pointer", userSelect: "none", padding: "8px", borderRadius: "8px", transition: "0.2s" }} onMouseOver={(e) => e.currentTarget.style.background = "#f8fafc"} onMouseOut={(e) => e.currentTarget.style.background = "transparent"}>
        ...
        <span style={{ color: checked ? "#005a9e" : "#475569", fontSize: "0.85rem", fontWeight: checked ? "600" : "500", transition: "color 0.2s" }}>{label}</span>
            ...
            <div style={{ flex: 1, overflowY: "auto", position: "relative", padding: "10px 0", background: "#fdfdfd" }}>
                ...
                {topPosition >= 0 && (
                    <div style={{
                        ...
                        <strong style={{ display: "block", fontSize: "0.8rem" }}>{horarioSelecionado}</strong>
                        <span style={{ opacity: 0.9 }}>Reserva Cirúrgica</span>
                    </div>
                )}
            </div>
        </div>
